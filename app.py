...
TABLE_FQN = "workspace.default.tallent_training_register"
...
def ensure_schema_and_seed(conn):
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            """
            INSERT INTO workspace.default.tallent_training_register (
...
def fetch_dashboard_data(conn):
    with conn.cursor() as cur:
        cur.execute(
            ...
            FROM workspace.default.tallent_training_register
            """
        )
        stats = cur.fetchone()
        cur.execute(
...
def ask_local_analytics(question, context_rows):
    ...
    # Similar training cohorts by recurring training_needs values.
    cohorts = {}
    for row in context_rows:
...
def ask_llm(question, context_rows):
    ...
    # default backend: databricks gateway
    try:
  ...
  <body>
    <main>
      ...
      <p style="margin-top:16px; color:#64748b;">
        Data persists in Delta table <code>workspace.default.tallent_training_register</code>.
      </p>
    </main>
  </body>
</html>
